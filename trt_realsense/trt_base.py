...
class TRTWrapper:
    ...
    def infer(self, feed_dict):
        ...
        for name in self.outputs:
            ...
            if current_binding is None or tuple(current_binding.shape) != tuple(
                out_shape
            ):
                dtype = self._trt_to_torch_dtype(self.engine.get_tensor_dtype(name))
                try:
                    self.io_bindings[name] = torch.empty(
                        tuple(out_shape), dtype=dtype, device="cuda"
                    )
                except:
...
class SAM2Base_TRT(torch.nn.Module):
    def __init__(
        self,
        trt_encoder,
        trt_decoder_mask,
        ...
        trt_memory_encoder,
        trt_obj_ptr_proj,
        trt_obj_ptr_tpos_proj,
        memory_attention,
        num_maskmem=7,  # default 1 input frame + 6 previous frames
        image_size=1024,
        backbone_stride=16,  # stride of the image backbone output
        sigmoid_scale_for_mem_enc=20.0,  # scale factor for mask sigmoid prob
        sigmoid_bias_for_mem_enc=-10.0,  # bias factor for mask sigmoid prob
        # During evaluation, whether to binarize the sigmoid mask logits on interacted frames with clicks
        binarize_mask_from_pts_for_mem_enc=True,
        use_mask_input_as_output_without_sam=True,  # on frames with mask input, whether to directly output the input mask without using a SAM prompt encoder + mask decoder
        ...
        # self.memory_attention = torch.compile(memory_attention, backend="cudagraphs")
        ...
        self.trt_decoder = trt_decoder_mask
        self.trt_decoder_points = trt_decoder_points
        self.trt_memory_encoder = trt_memory_encoder
        self.trt_obj_ptr_proj = trt_obj_ptr_proj
        self.trt_obj_ptr_tpos_proj = trt_obj_ptr_tpos_proj
    ...
    def _forward_sam_heads(
        ...
        if mask_inputs is not None:
            ...
            if mask_inputs.shape[-2:] != (self.high_res_mask, self.high_res_mask):
                sam_mask_prompt = F.interpolate(
                    mask_inputs.float(),
                    size=(self.high_res_mask, self.high_res_mask),
                    align_corners=False,
                    mode="bilinear",
                ...
                decoder_outputs = self.trt_decoder.infer(decoder_inputs)
            ...
            decoder_outputs = self.trt_decoder_points.infer(decoder_inputs)

        low_res_multimasks = decoder_outputs["low_res_multimasks"]
        ious = decoder_outputs["ious"]
        sam_output_tokens = decoder_outputs["sam_output_tokens"]
        ...
        if multimask_output:
            ...
            low_res_masks, high_res_masks = (
                low_res_multimasks[:, :1],
                high_res_multimasks[:, :1],
            )

        # Extract object pointer from the SAM output token (with occlusion handling)
        proj_inputs = {"sam_output_tokens": sam_output_token}
        obj_ptr = self.trt_obj_ptr_proj.infer(proj_inputs)
        obj_ptr = obj_ptr["proj_obj_tokens"]
        ...
        if self.pred_obj_scores:
            ...
            obj_ptr = obj_ptr + (1 - lambda_is_obj_appearing) * self.no_obj_ptr.to(
                device
            )
    ...
    def forward_image(self, img_batch: torch.Tensor):
        ...
        encoder_inputs = {"image": img_batch}
        out = self.trt_encoder.infer(encoder_inputs)
        backbone_out = {
            "vision_features": [out["vision_features"]],  # 1, 256, 64, 64
            "vision_pos_enc": [out["vision_pos_enc"]],  # 1, 256, 64, 64
            "backbone_fpn": [
                out["high_res_0"],
                out["high_res_1"],
    ...
    def _prepare_backbone_features(self, backbone_out):
        ...
        feature_maps = backbone_out["vision_features"]
            ...
            if self.use_obj_ptrs_in_encoder:
                ...
                if len(pos_and_ptrs) > 0:
                    ...
                    if self.add_tpos_enc_to_obj_ptrs:
                        ...
                        proj_inputs = {"obj_pos": obj_pos.unsqueeze(1)}
                        obj_pos = self.trt_obj_ptr_tpos_proj.infer(proj_inputs)
                        obj_pos = obj_pos["obj_pos_proj"]
    ...
    def _encode_new_memory(
        ...
        memory_inputs = {"pix_feat": pix_feat, "mask_for_mem": mask_for_mem}
        maskmem_out = self.trt_memory_encoder.infer(
            memory_inputs
        )  # sigmoid already applied

        maskmem_features = maskmem_out["memory_features"]
        maskmem_pos_enc = [maskmem_out["memory_pos_enc"]]
...
class SAM2VideoPredictor_TRT(SAM2Base_TRT):
    ...
    @classmethod
    def from_pretrained(cls, model_id: str, **kwargs) -> "SAM2VideoPredictor_TRT":
        """
    ...
    def _get_image_feature(self, inference_state, frame_idx, batch_size):
        ...
        expanded_backbone_out = {
            ...
            "vision_features": backbone_out["vision_features"].copy(),
        }
    ...
    def _run_single_frame_inference(
        ...
        current_out = self.track_step(
            ...
            prev_sam_mask_logits=prev_sam_mask_logits,
            high_res_features=backbone_out["backbone_fpn"],
        )
...
def benchmark_interaction(name, device, func, *args, **kwargs):
    """Helper to time a specific function call."""
    if device.type == "cuda":
        torch.cuda.synchronize()  # Ensure GPU work is finished before starting timer
    ...
    print(f"{name: <25} | {duration:6.2f} ms | {fps:6.1f} FPS")
    return result
